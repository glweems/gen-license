...
console.log(readPackageJson("package.json", "name"));
...
(async function () {
  ...
  const initialLicense = licenses.findIndex(
  ...
  const promptLicense: PromptObject<"license"> = {
    ...
    choices: licenses,
    initial: initialLicense >= 0 ? initialLicense : 0,
  };
  ...
  if (initialLicense > 0) questions.push(promptLicense);
